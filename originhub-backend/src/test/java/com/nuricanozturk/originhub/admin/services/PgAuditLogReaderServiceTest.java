...
class PgAuditLogReaderServiceTest {
  ...
  private static final String SAMPLE_LINE =
      "2026-06-05 10:15:30.123 UTC [42]: [1-1] user=admin,db=originhub,app=[unknown],client=172.18.0.2"
  ...
  void parseLine_parsesStandardPgAuditLine() {
    ...
    assertThat(entry.database()).isEqualTo("originhub");
  ...
  void parseLine_parsesLineWithoutConnectionInfo() {
    ...
    assertThat(entry.statement()).contains("CREATE DATABASE \"originhub\"");
  }
...
""";
  ...
  void search_appliesFilters(@TempDir Path tempDir) throws Exception {
    when(adminPlatformSettingsService.isPgAuditViewerEnabled()).thenReturn(true);
    var ddlLine =
        "2026-06-05 10:16:00.000 UTC [43] user=postgres,db=originhub LOG:  AUDIT: SESSION,2,1,DDL,CREATE,TABLE,public.audit_logs,\"CREATE TABLE audit_logs (...)\"";
    Files.writeString(
        tempDir.resolve("postgresql-2026-06-05.log"),
