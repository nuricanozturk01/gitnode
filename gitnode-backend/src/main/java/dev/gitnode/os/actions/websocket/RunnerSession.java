...
public class RunnerSession {
  ...
  public synchronized void sendRaw(final String json) {
    if (!this.session.isOpen()) {
      log.warn("Cannot send raw message to runner {}: session closed", this.runnerId);
      return;
    }
    try {
      this.session.sendMessage(new TextMessage(json));
    } catch (final IOException ex) {
      log.warn("Failed to send raw message to runner {}", this.runnerId, ex);
    }
  }
