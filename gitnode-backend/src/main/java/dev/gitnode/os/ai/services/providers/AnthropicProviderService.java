...
public class AnthropicProviderService implements AiProviderService {
  ...
  @Override
  @CircuitBreaker(name = "ai-anthropic", fallbackMethod = "fallback")
  ...
  @Override
  @CircuitBreaker(name = "ai-anthropic", fallbackMethod = "fallbackWithTokens")
  public String complete(
    ...
    if (apiKey == null || apiKey.isBlank()) {
      throw new ErrorOccurredException("Anthropic API key not configured");
    }
    final var model =
        (settings.getModel() != null && !settings.getModel().isBlank())
            ? settings.getModel()
            : AnthropicChatOptions.DEFAULT_MODEL;
    ...
    final var chatModel = AnthropicChatModel.builder().options(options).build();
  ...
  private String fallback(
      ...
      final String userPrompt,
      final Throwable t) {
    log.warn("Anthropic circuit breaker open: {}", t.getMessage());
