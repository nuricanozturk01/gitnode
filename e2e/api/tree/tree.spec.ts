...
test.describe.serial('Tree API — all endpoints', () => {
  test('GET /api/repos/{owner}/{repo}/tree/{branch}', async ({ authedRequest, api }) => {
    const response = await authedRequest.get(`${reposApi(api.owner, api.repo)}/tree/main`);
    expect(response.status()).toBe(200);
    const body = await response.json();
    expect(body.entries).toBeDefined();
  });

  test('GET /api/repos/{owner}/{repo}/blob/{branch}/**', async ({ authedRequest, api }) => {
  ...
  test('GET /api/repos/{owner}/{repo}/raw/{branch}/**', async ({ authedRequest, api }) => {
    const response = await authedRequest.get(`${reposApi(api.owner, api.repo)}/raw/main/README.md`);
    expect(response.status()).toBe(200);
  ...
  test('GET /api/repos/{owner}/{repo}/languages', async ({ authedRequest, api }) => {
    const response = await authedRequest.get(`${reposApi(api.owner, api.repo)}/languages`, {
      params: { branch: 'main' },
    });
    expect(response.status()).toBe(200);
  ...
  test('GET /api/repos/{owner}/{repo}/archive/{branch}', async ({ authedRequest, api }) => {
    const response = await authedRequest.get(`${reposApi(api.owner, api.repo)}/archive/main`);
    expect(response.status()).toBe(200);
    expect(response.headers()['content-type']).toContain('zip');
  });

  test('PUT /api/repos/{owner}/{repo}/blob/{branch}/**', async ({ authedRequest, api }) => {
