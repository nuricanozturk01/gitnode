...
export class TagService {
  ...
  get(owner: string, repo: string, tagName: string): Promise<TagInfo> {
    return firstValueFrom(this.http.get<TagInfo>(`${this.base(owner, repo)}/${tagName}`));
  }

  create(owner: string, repo: string, form: { name: string; sha?: string; message?: string }): Promise<TagInfo> {
    return firstValueFrom(this.http.post<TagInfo>(this.base(owner, repo), form));
  }

  delete(owner: string, repo: string, tagName: string): Promise<void> {
