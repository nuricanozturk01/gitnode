 ...
 */
package dev.gitnode.os.shared.repo.repositories;
...
public interface RepoRepository extends JpaRepository<Repo, UUID> {
  ...
  @Query(
      "SELECT new dev.gitnode.os.shared.repo.repositories.RepoStatRow("
          + "r.owner.username, r.name, r.createdAt) "
          + "FROM Repo r JOIN r.owner WHERE r.createdAt >= :since")
  List<RepoStatRow> findRepoStatsSince(@Param("since") Instant since);
