...
public class PrHeadBranchCleanupListener {
  ...
  public void onPullRequestStatusChanged(final @NonNull PullRequestStatusChangedEvent event) {
    ...
    final var repo = this.repoRepository.findByIdWithOwner(event.repoId()).orElse(null);
    if (repo == null) {
      return;
    }

    final var enabled =
        isMerged && repo.isDeleteHeadBranchOnPrMerge()
    ...
    try {
      ...
      log.debug(
          "Deleted head branch {} on {} after PR {} ({})", source, repoName, event.prId(), status);
    } catch (final ItemNotFoundException e) {
      log.debug(
          "Head branch {} not found on {} after PR {}: {}",
          source,
          repoName,
          event.prId(),
          e.getMessage());
